...
def handle_match_command(respond, ack, body, client, logger):
    ...
    post_message(client, body["channel_id"], f"<@{body['user_id']}> starts by choosing a block! Please choose your block outside the thread due to Slack bot limitations.", thread_ts=timestamp)
    match_id += 1
    matches[match_id] = {"initiator": body["user_id"], "matched": chosen, "ts": timestamp, "stage": 0, "Block 1": None, "Block 2": None, "Points": [0, 0]}
            ...
            if matchx["ts"] == info.get("thread_ts"):
                if info["user"] == matchx["initiator"] and matchx["stage"] == 0 and matchx["Block 1"] in caseify(info["text"]):
                    ...
                    post_message(
                        client,
                        info["channel"],
                        f"<@{matchx['matched']}> has guessed the block! <@{matchx['initiator']}> gets {0} points. It's <@{matchx['matched']}>'s turn to choose a block now!",
                        matchx["ts"]
                    )
                    matchx["stage"] += 1
